...
(function() {
  ...
  Parser = (function(_super) {
    ...
    function Parser(arg, callback, opts) {
      ...
      //window.onFileMatched("inside 4");
      if (typeof this.arg === 'string') {
        //window.onFileMatched("inside 5");
        ...
        setTimeout(loader, 1);
        //window.onFileMatched("inside 15");
      }
    }
    ...
    Parser.prototype.readBlock = function(block) {
      ...
      //window.onFileMatched("inside 7.9 " + rows + " :: " + this.LINEWIDTH);
      while (rows--) {
        ...
        if (arr[rowIndex] === 69 && arr[rowIndex + 1] === 78 && arr[rowIndex + 2] === 68 && arr[rowIndex + 3] === 32) {
          ...
          //window.onFileMatched("inside 7.91")
          header = new Header(s);
          ...
          //window.onFileMatched("inside 7.92");
          this.hdus.push(new HDU(header, dataunit));
          this.offset += this.end + dataLength + this.excessBytes(dataLength);
          if (this.offset === this.length) {
            //window.onFileMatched("inside 7.93 ")
            this.headerStorage = null;
            //window.onFileMatched(this.callback);
            this.invoke(this.callback, this.opts, this);
            //window.onFileMatched("inside 7.931");
            return;
          }
          //window.onFileMatched("inside 7.94")
          this.blockCount = 0;
          this.begin = this.blockCount * this.BLOCKLENGTH;
          this.end = this.begin + this.BLOCKLENGTH;
          this.headerStorage = new Uint8Array();
          block = this.arg.slice(this.begin + this.offset, this.end + this.offset);
          //window.onFileMatched("inside 7.95");
          this.readNextBlock(block);
          //window.onFileMatched("inside 7.96");
          return;
        }
        break;
      }
      //window.onFileMatched("inside 8");

      this.blockCount += 1;
      this.begin = this.blockCount * this.BLOCKLENGTH;
      this.end = this.begin + this.BLOCKLENGTH;
      block = this.arg.slice(this.begin + this.offset, this.end + this.offset);
      //window.onFileMatched("inside 8.01");

      this.readNextBlock(block);
      //window.onFileMatched("inside 9");

    };

    Parser.prototype._readBlockFromBuffer = function(block) {
      //window.onFileMatched("inside 8.1");
      return this.readBlock(block);
    };
  ...
  FITS = (function(_super) {
    ...
    function FITS(arg, callback, opts) {
      var parser,
        _this = this;
      this.arg = arg;
      //window.onFileMatched("inside 1");
      parser = new Parser(this.arg, function(fits) {
        //window.onFileMatched("inside 2");
  ...
  Image = (function(_super) {
      ...
      if (bitpix > 0) {
        ...
        if (bitpix == -32) {
            ...
            while(i--) {
                val1 = arr[2*i];
                val2 = arr[2*i - 1];
                arr[2*i] = swapEndian(val2);
                arr[2*i - 1] = swapEndian(val1);
            }
    ...
    Image.prototype.getFrame = function(frame, callback, opts) {
      ...
      if ((buffers != null ? buffers.length : void 0) === this.nBuffers) {
        //window.onFileMatched("Getting frame async");
        return this._getFrameAsync(buffers, callback, opts);
      } else {
        //window.onFileMatched("Getting frame sync");
        ...
        //window.onFileMatched("Try stuff");
