...
FFT.prototype.conjugate = function() {
    for (var i = 0; i < this.imag.length; i++) {
        this.imag[i] *= -1;
    }
};
FFT.prototype.multiply = function(fft2) {
    var r = 0, i = 0;
    var real = [];
    var imag = [];
    for (var j = 0; j < this.real.length; j++) {
        r = this.real[j] * fft2.real[j] - this.imag[j] * fft2.imag[j];
        i = this.real[j] * fft2.imag[j] + this.imag[j] * fft2.real[j];
        real.push(r);
        imag.push(i);
    }
    var result = new FFT(this.real.length, this.real.length);
    result.real = real;
    result.imag = imag;
    return result;
};
