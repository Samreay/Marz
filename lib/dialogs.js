...
angular.module('dialogs.controllers',['ui.bootstrap.modal','pascalprecht.translate'])
	...
	.config(['$translateProvider',function($translateProvider){
		$translateProvider.translations('en-US',{
            ...
            DIALOGS_INPUT: "Input Required",
            DIALOGS_INPUT_MSG: "Please enter the required input.",
    ...
    .controller('inputDialogCtrl',['$scope', '$modalInstance', '$translate','header','msg','data', function($scope,$modalInstance,$translate,header,msg,data){

        $scope.header = (angular.isDefined(header)) ? header : $translate.instant('DIALOGS_INPUT');
        $scope.msg = (angular.isDefined(msg)) ? msg : $translate.instant('DIALOGS_INPUT_MSG');
        $scope.data = {};
	...
	.provider('dialogs',[function(){
		...
		this.$get = ['$modal',function ($modal){
			...
			return {
                 ...
                 * @param	opts	object
                 */
                input : function(header,msg,data,opts){
                    opts = _setOpts(opts);

                    return $modal.open({
                        templateUrl : '/dialogs/input.html',
...
angular.module('dialogs.main',['dialogs.services','ngSanitize']) // requires angular-sanitize.min.js (ngSanitize) //code.angularjs.org/1.2.1/angular-sanitize.min.js
	...
	.run(['$templateCache','$interpolate',function($templateCache,$interpolate){
        ...
        $templateCache.put('/dialogs/input.html','<div class="modal-header"><h4 class="modal-title" ng-bind-html="header"></h4></div><div class="modal-body"><ng-form name="nameDialog" novalidate role="form"><div class="form-group input-group-lg" ng-class="{true: \'has-error\'}[nameDialog.data.$dirty && nameDialog.data.$invalid]"><label ng-bind-html="msg" class="control-label" for="course"></label><input type="text" class="form-control" name="data" id="data" ng-model="data.value" ng-keyup="hitEnter($event)" required></div></ng-form></div><div class="modal-footer"><button type="button" class="btn btn-default" ng-click="cancel()">Cancel</button><button type="button" class="btn btn-primary" ng-click="save()" ng-disabled="(nameDialog.$dirty && nameDialog.$invalid) || nameDialog.$pristine">Save</button></div>');
